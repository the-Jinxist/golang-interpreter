...
// These spell out the operator precedence
const (
	_ int = iota
	LOWEST
	EQUALS      // ==
...
var precedences = map[token.TokenType]int{
	token.EQUALS_TO:     EQUALS,
	token.NOT_EQUALS_TO: EQUALS,
...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerInfix(token.EQUALS_TO, p.parseInfixExpression)
	p.registerInfix(token.NOT_EQUALS_TO, p.parseInfixExpression)
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	if !p.expectPeekAndNext(token.IDENT) {
		return nil
	}
...
func (p *Parser) expectPeekAndNext(t token.TokenType) bool {
	if p.peekTokenIs(t) {
		p.nextToken()
		return true
	} else {
...
func (p *Parser) parseFunctionLiteral() ast.Expression {
	funcLiteral := &ast.FunctionLiteral{Token: p.curToken, Parameters: []*ast.Identifier{}}
